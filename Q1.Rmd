...
a. Scrutinize the data to assess structure and quality. Are there any improbable or problematic entries? Provide a summary of checks performed and edit the data so entries are valid and meaningful where editing is reasonable to do.
...
c. We want to investigate what the differences are between the job listings for those under different classification, i.e. business analytics, data analytics and data science. Compare across the classifications using appropriate graphics the:   
...
# Mutate Salary interval by Salary_Min and Salary_Max
 ...
 Combined_Job_clean %>% 
   ...
   mutate(Salary_Max = max(Salary_Max),
...
From the above, it is found that data scientist had the largest variation in salary whereas data analyst had smallest. Moreover, both the lowest and highest salaries are found in Data Scientist category.
...
d. Your friend suspects that if an employer provides a salary range for the job, the salary is large and hence more attractive to potential candidates. Investigate this claim. Your investigation should be supported by graphics.
...
e. Is the location (via by State) associated with the salary and/or sector? Show graphics to best your conclusion.
